//
//  StringsDictGenerator.m
//  R.objc
//
//  Created by dev on 04/06/2018.
...
@interface StringDictResource: NSObject
...
@implementation StringDictResource
...
@interface StringsDictGenerator ()
@property (nonatomic, strong) NSMutableDictionary<NSString*, StringDictResource*>* translationsByPath;
@end

@implementation StringsDictGenerator
...
{
    NSArray* stringsFiles = [self.finder filesWithExtension:@"stringsdict"];
...
- (void)mapTranslationsByPathFromFiles:(NSArray<NSURL*>*)files error:(NSError *__autoreleasing *)error
{
    ...
    {
        StringDictResource* resource = [[StringDictResource alloc] initWithURL:url];
        if (!resource)
        {
        ...
        {
            StringDictResource* oldRes = self.translationsByPath[resource.filename];
        ...
        else
        {
            if (resource.fileContent.count > 0)
            {
                ...
                [CommonUtils logVerbose:@"Stringsdict file %@ with locale %@ found", resource.filename, resource.locale];
...
{
    for (StringDictResource* res in self.translationsByPath.allValues)
    {
...
- (RComment*)commentForStringResource:(StringDictResource*)res key:(NSString*)key
{
